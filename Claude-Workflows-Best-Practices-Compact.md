...
- **2 · Exploration** — mandatory before creative work: superpowers `brainstorming`. Ask only, propose nothing · search existing code for prior solutions, list reuse candidates (none → say so, don't assume) → can answer: exact problem? constraints? what different?
- **3 · Spec** — behavior · interfaces · edge cases · **transactional integrity** (atomic vs eventual) · assumptions · **access-control matrix** · **expected scale** · **reuse** (name existing code built on) · **presentation vs logic** (colors/copy/thresholds/flags in CSS/config, never inline). If auth tokens/sessions: ask *"stolen credential revocable instantly?"* → spec verified vs codebase, no unverified assumptions → user approves.
...
- **5 · Adversarial Review** — break plan. + **security abuse pass** (IDOR, RLS, authz, unvalidated input) + **scale pass** (what melts at peak ×10) + **reinvention/hardcode pass** (reuse existing code; values in config not inline). Loop back: surface → P4, architectural → P3, wrong problem → P2; >⅓ steps reworked → restart P3 → structurally sound → user approves.
...
- **8 · Post-Impl Review** — plan vs reality; flag doc updates · duplicated logic / inlined config value (only visible once written). No divergence found → say so explicitly, don't leave it implied by silence → reported → user sign-off.
